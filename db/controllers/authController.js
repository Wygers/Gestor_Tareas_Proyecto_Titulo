...
authController.showLogin = (req, res) => {
    ...
    res.render('login', { title: 'Iniciar Sesión - Gestor de Tareas', error, success });
};
...
authController.showRegistro = (req, res) => {
    ...
    delete req.session.error;
    delete req.session.success;
    res.render('registro', { title: 'Registro - Gestor de Tareas', error, success });
};
...
authController.login = async (req, res) => {
    ...
    try {
        if (!correo || !contrasena) {
            req.session.error = 'Debe completar todos los campos obligatorios.';
        ...
        if (usuarios.length === 0) {
            req.session.error = 'Credenciales inválidas o cuenta inactiva.';
            return res.redirect('/auth/login');
        }
...
authController.registrarUsuario = async (req, res) => {
    ...
    try {
        ...
        req.session.success = 'Organización y usuario administrador registrados exitosamente.';
        res.redirect('/auth/login');

    } catch (error) {
        await connection.rollback();
        console.error(error);
        req.session.error = 'Error al registrar. Es posible que el correo o RUT de la organización ya estén registrados.';
